...
import { AVISO, AVISO_OSCURO, AVISO_TENUE, ERROR, EXITO, NEUTRO_300, NEUTRO_400, NEUTRO_900, SUPERFICIE_3, SUPERFICIE_4 } from '../../theme/marca';
...
function catInfo(v: number) {
  ...
  if (v >= 80) return { label: 'Bueno',         color: marcaViva.taupeMedio, bg: SUPERFICIE_3 };
...
// ── stat card ─────────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, color = marcaViva.taupeMedio, subColor }: {
    ...
    <Box>
        ...
        {dist && (
          <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
            ...
            <Chip label={`${dist.avgPts.toFixed(1)} pts promedio`} size="small"
              sx={{ bgcolor: SUPERFICIE_3, color: marcaViva.taupeMedio, fontWeight: 700, fontSize: '0.78rem',
      ...
      <Grid container spacing={2} mb={3}>
        {[
          ...
          { label: 'Total RMs',     value: dist?.total ?? '—',   sub: '— Equipo completo',  color: marcaViva.taupe },
          { label: 'Prom. MIP',     value: dist ? dist.avgPts.toFixed(1) : '—',
            ...
            color: dist ? catColor(dist.avgPts) : marcaViva.taupeMedio,
            ...
            <Card elevation={0} sx={{ border: '1px solid #e0e7ef', borderRadius: 2 }}>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                  <Box sx={{ width: 3, height: 20, bgcolor: marcaViva.taupeMedio, borderRadius: 2 }} />
                ...
                <Box sx={{ display: 'flex', gap: 2, mt: 1.5, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {[
                    ...
                    { label: 'Bueno 80–89%',        color: marcaViva.taupeMedio, bg: SUPERFICIE_3 },
              ...
              <Card elevation={0} sx={{ border: '1px solid #e0e7ef', borderRadius: 2, height: '100%' }}>
                <CardContent>
                  ...
                  <DistCard label="Bueno"         count={dist.buenos}       total={dist.total}
                    rangoLabel="80–89 puntos MIP" color={marcaViva.taupeMedio} bg={SUPERFICIE_3} miembros={dist.miembros.bueno} />
                  ...
                  <Box sx={{ p: 1.5, bgcolor: '#f8f9fa', borderRadius: 1.5, border: '1px solid #EFEBE6' }}>
                    <Typography variant="caption"
                      sx={{ color: marcaViva.taupe, fontWeight: 700, textTransform: 'uppercase',
