...
Los rangos en DIM_IndicadorTabla son por (indicador_id, pais_id), por lo que
...
def convertir_a_puntaje(
    ...
    - Si se provee pais_id, filtra por país (comportamiento correcto multi-país).
    ...
    if pais_id is not None:
        q = q.filter(IndicadorTabla.pais_id == pais_id)

    tablas = q.order_by(IndicadorTabla.rango_desde.asc()).all()

    if not tablas:
        logger.debug(
            f"Indicador {indicador_id} / país {pais_id}: sin tabla de conversión — "
    ...
    for tabla in tablas:
        if tabla.rango_desde <= valor <= tabla.rango_hasta:
            logger.debug(
                f"Indicador {indicador_id} / país {pais_id}: valor={valor} → "
    ...
    if valor > tablas[-1].rango_hasta:
        logger.debug(
            f"Indicador {indicador_id} / país {pais_id}: valor={valor} supera máximo "
            f"{tablas[-1].rango_hasta} → puntos máximos={tablas[-1].puntos}"
        )
        return tablas[-1].puntos

    logger.debug(
        f"Indicador {indicador_id} / país {pais_id}: valor={valor} por debajo del mínimo "
...
def convertir_puntaje_por_codigo(
    ...
    if not indicador:
        logger.warning(
            f"Indicador '{indicador_codigo}' (país {pais_id}) no encontrado para conversión de puntaje"
        )
        return _clamp(valor, Decimal("0"), Decimal("100"))

    return convertir_a_puntaje(db, indicador.id, valor, pais_id)
