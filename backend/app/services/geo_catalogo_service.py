...
def listar_provincias(db: Session, pais_codigo: str | None = None) -> list[dict]:
    q = db.query(Provincia).filter(Provincia.activo == True)  # noqa: E712
    if pais_codigo:
...
def listar_municipios(db: Session, provincia_id: int | None = None) -> list[dict]:
    q = db.query(Municipio).filter(Municipio.activo == True)  # noqa: E712
    if provincia_id:
...
def listar_centros(db: Session, pais_codigo: str | None = None) -> list[dict]:
    q = db.query(CentroMedico).filter(CentroMedico.activo == True)  # noqa: E712
    if pais_codigo:
...
def listar_especialidades(db: Session) -> list[dict]:
