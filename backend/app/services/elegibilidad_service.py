...
from app.models.hechos import ResultadoIndicador, Coaching, CapacitacionFact, Ventas
from app.models.dimensiones import Indicador


def evaluar_elegibilidad_rm(
    db: Session, rm_id: int, pais_codigo: str, ciclo_id: Optional[int] = None
) -> dict:
    """
    ...
    q = db.query(ReglaElegibilidad).filter(
        ReglaElegibilidad.pais_codigo == pais_codigo,
    ...
    for regla in reglas:
        valor_actual = _obtener_valor_indicador(db, rm_id, pais_codigo, ciclo_id, regla.indicador_codigo)
...
def _obtener_valor_indicador(
    db: Session, rm_id: int, pais_codigo: str, ciclo_id: Optional[int], indicador_codigo: str
) -> Decimal:
    """Obtiene el valor real del indicador para el RM dado."""

    # Indicadores de productividad (FACT_ResultadoIndicador — antes RendimientoComercial)
    prod_codigos = {"COBERTURA_F1", "COBERTURA_F2", "COBERTURA_FARMACIAS", "PROMEDIO_DIARIO"}
    if indicador_codigo.upper() in prod_codigos:
        q = db.query(func.avg(ResultadoIndicador.resultado_porcentaje)).join(
    ...
    if indicador_codigo.upper() == "COACHING":
        q = db.query(func.avg(Coaching.cumplimiento_pct)).filter(
            Coaching.rm_id == rm_id, Coaching.pais_codigo == pais_codigo
        )
    ...
    if indicador_codigo.upper() == "CAPACITACION":
        total = db.query(func.count(CapacitacionFact.id)).filter(
            CapacitacionFact.rm_id == rm_id, CapacitacionFact.pais_codigo == pais_codigo
        )
    ...
    if indicador_codigo.upper() == "COMERCIAL":
        q = db.query(func.avg(Ventas.cumplimiento_pct)).filter(
            Ventas.rm_id == rm_id, Ventas.pais_codigo == pais_codigo
        )
