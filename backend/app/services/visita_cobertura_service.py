...
def proyeccion(db: Session, ciclo_id: int | None = None, vm_id: int | None = None,
               dia_actual: int | None = None, obj_pct: float = OBJ_COBERTURA / 100) -> dict:
    """Proyección de cobertura al cierre del ciclo (spec Parte 7)."""
    ciclo_id = ciclo_id or ciclo_por_defecto(db)
    ciclo = db.query(Ciclo).filter(Ciclo.id == ciclo_id).first() if ciclo_id else None
    ciclo_dias = _dias_habiles(getattr(ciclo, "fecha_inicio", None), getattr(ciclo, "fecha_fin", None))
    if dia_actual is None:
        dia_actual = _dia_actual_ciclo(ciclo) if ciclo else min(8, ciclo_dias)
    dia_actual = max(1, min(int(dia_actual), ciclo_dias))
    dias_restantes = max(0, ciclo_dias - dia_actual)

    base = _cobertura_base(db, ciclo_id, vm_id) if ciclo_id else {"panel": 0, "visitados": 0, "categorias": {}}
    panel = base["panel"]
    visitados = base["visitados"]
    obj_medicos = round(panel * obj_pct)

    ritmo_actual = round(visitados / dia_actual, 2) if dia_actual else 0.0
    proyeccion_final = round(min(panel, visitados + ritmo_actual * dias_restantes))
    gap = obj_medicos - proyeccion_final
    ritmo_requerido = round(max(0.0, (obj_medicos - visitados) / dias_restantes), 2) if dias_restantes else (
        0.0 if visitados >= obj_medicos else None)

    categorias = {}
    for c, cat in base.get("categorias", {}).items():
        rc = round(cat["visitados"] / dia_actual, 2) if dia_actual else 0.0
        categorias[c] = {
            "panel": cat["total"], "visitados": cat["visitados"],
            "proyeccion": round(min(cat["total"], cat["visitados"] + rc * dias_restantes)),
        }

    return {
        "ciclo_id": ciclo_id, "ciclo_dias": ciclo_dias, "dia_actual": dia_actual,
        "dias_restantes": dias_restantes, "panel": panel, "visitados": visitados,
        "objetivo_pct": round(obj_pct * 100, 1), "obj_medicos": obj_medicos,
        "ritmo_actual": ritmo_actual, "proyeccion_final": proyeccion_final,
        "gap_al_objetivo": gap, "ritmo_requerido": ritmo_requerido,
        "cumple_proyeccion": proyeccion_final >= obj_medicos,
        "categorias": categorias,
    }


def ranking_proyeccion(db: Session, ciclo_id: int | None = None, dia_actual: int | None = None) -> dict:
    """Detalle desplegable de Proyección: por VM, la proyección final y si alcanzará el objetivo."""
    ciclo_id = ciclo_id or ciclo_por_defecto(db)
    vm_ids = [r[0] for r in db.query(MedicoVisita.vm_id)
              .filter(MedicoVisita.activo == True).distinct().all()]  # noqa: E712
    if not vm_ids:
        return {"metrica": "proyeccion", "objetivo": None, "items": [], "no_cumplen": 0, "total": 0}
    nombres = dict(db.query(RepresentanteMedico.id, RepresentanteMedico.nombre)
                   .filter(RepresentanteMedico.id.in_(vm_ids)).all())
    zonas = dict(db.query(RepresentanteMedico.id, RepresentanteMedico.zona)
                 .filter(RepresentanteMedico.id.in_(vm_ids)).all())
    filas = []
    for vm in vm_ids:
        p = proyeccion(db, ciclo_id, vm, dia_actual)
        pct = _pct(p["proyeccion_final"], p["panel"]) if p["panel"] else 0.0
        filas.append({"vm_id": vm, "nombre": nombres.get(vm, f"VM #{vm}"), "zona": zonas.get(vm),
                      "valor": pct, "cumple": p["cumple_proyeccion"]})
    filas.sort(key=lambda f: f["valor"])  # peor primero
    return {"metrica": "proyeccion", "objetivo": OBJ_COBERTURA,
            "no_cumplen": sum(1 for f in filas if not f["cumple"]),
            "total": len(filas), "items": filas}
