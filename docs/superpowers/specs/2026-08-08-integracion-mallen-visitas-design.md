...
**`PROM_DIARIO` va CRUDO — médicos distintos ÷ días laborables, sin normalizar.** Es literalmente el §2.1, y los datos de producción lo confirman: los históricos que cargaba el Excel van de **0.50 a 1.11**, con `resultado_porcentaje` de **50 a 100**. Ese es exactamente el rango de la tasa cruda cuando el numerador son médicos **distintos** del ciclo (un RM cubre ~40-60 fichas distintas en ~40 días laborables → ratio ~1.0-1.4). El motor lo multiplica por 100 y satura arriba, que es el comportamiento correcto: quien supera el objetivo llega al tope.

> **Corrige un error de análisis mío.** Una versión anterior de este spec normalizaba `PROM_DIARIO` dividiéndolo entre `DIM_MetaIndicador`. Partía de un ejemplo sintético (1 médico en 20 días = 0.05) que hacía parecer absurda la tasa cruda; con volumen real no lo es. Peor: en la base **`meta_100 = 100` en los ocho indicadores** — es un marcador de escala, no un objetivo — así que dividir hundía el valor. Medido: un RM con 45 médicos en 40 días pasaba de ~100% a **1.12%**, perdiendo ~10 puntos de Score **sin emitir un solo hallazgo**, porque el guard "sin meta no escribo" nunca disparaba. La lección: **antes de añadir una normalización, mirar el rango de los datos históricos** — ahí estaba escrita la unidad que el sistema esperaba.
